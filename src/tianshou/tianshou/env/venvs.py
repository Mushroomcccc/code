...
class BaseVectorEnv(object):
    ...
    def step(
        ...
        if self.is_hrl:
            obs_list, l_rew_list, h_rew_list, term_list, trunc_list, info_list = tuple(zip(*result))
            try:
            ...
            obs_list, rew_list, _, term_list, trunc_list, info_list = tuple(zip(*result))
            try:
                obs_stack = np.stack(obs_list)
            except ValueError:  # different len(obs)
                obs_stack = np.array(obs_list, dtype=object)
...
class DummyVectorEnv(BaseVectorEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], ENV_TYPE]], is_hrl=False, **kwargs: Any) -> None:
        super().__init__(env_fns, DummyEnvWorker, is_hrl=is_hrl, **kwargs)
