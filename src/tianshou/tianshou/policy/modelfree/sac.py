...
#         # >>> CHANGED: 连续动作维度（注意：这里 n_actions 表示 action_dim）
#         action_dim = int(n_actions)
#         assert action_dim >= 1, "n_actions 应为连续动作维度（>=1）"

#         # >>> CHANGED: SAC 标准做法的目标熵（负数）
#         self._target_entropy = -float(action_dim) * float(target_entropy_ratio)
...
#             state=hidden,
#             dist=dist,
#             log_prob=log_prob,   # 供 actor loss 与 alpha loss 使用
#             obs_emb=obs_emb,
#             entroy=entroy        # 仅作观测/日志，不再用于 alpha
#         )

#     def _target_q(self, batch, buffer: ReplayBuffer, indices: np.ndarray) -> torch.Tensor:
#         # >>> CHANGED: 明确切断目标分支的梯度
#         with torch.no_grad():
#             obs_next_result = self(batch, buffer, indices=indices, input="obs_next", is_train=False)
...
#             nn.utils.clip_grad_norm_(self.state_tracker.parameters(), max_norm=self._grad_norm)
#         self.actor_optim.step()
#         if self.optim_state is not None:
#             self.optim_state.step()

#         # >>> CHANGED: α 的自适应——使用带 Tanh 修正后的 log_prob（与 actor/target 使用量一致）
#         if self._is_auto_alpha:
#             # detach 只让 α 自己的梯度更新
#             alpha_loss = (self._log_alpha * (obs_result.log_prob.detach() + self._target_entropy)).mean()
#             self._alpha_optim.zero_grad()
#             alpha_loss.backward()
#             self._alpha_optim.step()
#             self._alpha = self._log_alpha.detach().exp()
#         else:
#             alpha_loss = torch.tensor(0.0, device=obs_emb.device)
...
class SACPolicy(DDPGPolicy):
    ...
    def _target_q(self, batch, buffer, indices):
        with torch.no_grad():
            obs_next_result = self(batch, buffer, indices=indices, input="obs_next", is_train=False)
            act_ = obs_next_result.act
            obs_next_emb = obs_next_result.obs_emb
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        if self._grad_norm:  # clip large gradient
            ...
            nn.utils.clip_grad_norm_(
                self.state_tracker.parameters(), max_norm=self._grad_norm
            )
        self.actor_optim.step()
        self.optim_state.step()
