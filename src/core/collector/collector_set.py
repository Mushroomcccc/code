...
from src.tianshou.tianshou.data import Batch, VectorReplayBuffer, ReplayBuffer, HVectorReplayBuffer
...
class CollectorSet(object):
    def __init__(self, policy, envs_dict, buffer_size, env_num,
        ...
        for name, envs in envs_dict.items():
            collector = Collector(
                policy, envs,
                HVectorReplayBuffer(buffer_size, env_num) if is_hrl else VectorReplayBuffer(buffer_size, env_num),
