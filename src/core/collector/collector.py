...
class Collector(object):
    ...
    def __init__(
        ...
        self.is_hrl = is_hrl
        self.env_step = np.zeros(self.env_num)
    ...
    def _assign_buffer(self, buffer: Optional[ReplayBuffer]) -> None:
        ...
        elif isinstance(buffer, HReplayBufferManager):
            assert buffer.buffer_num >= self.env_num
    ...
    def reset(
        ...
        :param bool reset_buffer: if true, reset the replay buffer that is attached
            to the collector.
        :param gym_reset_kwargs: extra keyworh_actd arguments to pass into the environment's
    ...
    def collect(
        ...
        if n_step is not None:
            ...
            self.env_step = self.env_step[:min(self.env_num, n_episode)]
        else:
            raise TypeError(
           ...
           # assert len(self.data) == len(ready_env_ids)
            ...
            if random:
                ...
                if self.is_hrl:
                    ...
                    self.data.update(env_step=np.copy(self.env_step))
                    self.env_step += 1
                    ...
                    if no_grad:
                        with torch.no_grad():  # faster than retain_grad version
                            ...
                            result = self.policy(self.data, self.buffer, indices=indices, is_obs=True, state=last_state, remove_recommended_ids=self.remove_recommended_ids, is_train=is_train,
                                                 use_batch_in_statetracker=True)
                    else:
                        result = self.policy(self.data, self.buffer, indices=indices, is_obs=True, state=last_state,
                                             remove_recommended_ids=self.remove_recommended_ids, is_train=is_train,
                                             use_batch_in_statetracker=True)
                    ...
                    h_act = to_numpy(result.h_act)
                    l_act = to_numpy(result.l_act)
                    if self.exploration_noise:
                        h_act, l_act = self.policy.exploration_noise(h_act, l_act)
                    self.data.update(policy=policy, act=l_act, h_act=h_act)
                else:
                    indices = self.buffer.last_index if len(self.buffer) > 0 else None
                    if indices is not None:
                        indices = indices[ready_env_ids] if n_episode else indices
            ...
            # 针对动作为虚拟物品编码的策略使用，将动作映射到一个最相似的真实物品
            action_remap = self.policy.map_action(self.data)  # RecPolicy transform!

            # 与环境进行交互
            if self.is_hrl:
                obs_next, rew, h_rew, terminated, truncated, info = self.env.step(action_remap, ready_env_ids)
                self.data.update(h_rew=h_rew)
            else:
                obs_next, rew, _, terminated, truncated, info = self.env.step(action_remap, ready_env_ids)
            ...
            if np.any(done):
                ...
                if n_episode:
                    ...
                    if surplus_env_num > 0:
                        ...
                        self.env_step = self.env_step[mask]
