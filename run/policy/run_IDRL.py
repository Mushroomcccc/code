...
from src.core.envs.Simulated_Env.hrl_intrinsic import HRLIntrinsicSimulatedEnv 
...
from src.tianshou.tianshou.utils.net.continuous import ActorProb
...
from src.tianshou.tianshou.policy import HA2CPolicy
...
def get_args_FHA2C():
    ...
    # Env
    parser.add_argument('--lambda_diversity', default=0.005, type=float)

    # Agent
    parser.add_argument('--lambda_guide', default=0.1, type=float)
    parser.add_argument("--model_name", type=str, default="IDRL")
    ...
    parser.add_argument('--k_near', default=1000, type=int)
    parser.add_argument('--subgoal_interval', default=4, type=int)
    parser.add_argument('--max_grad_norm', default=2., type=float)


    parser.add_argument("--message", type=str, default="IDRL")
...
def prepare_train_envs_local(args, ensemble_models, env, dataset, kwargs_um):
    ...
    train_envs = DummyVectorEnv(
        [lambda: HRLIntrinsicSimulatedEnv(**kwargs) for _ in range(args.training_num)], is_hrl=True)
...
def setup_policy_model(args, state_tracker, train_envs, test_envs_dict):
    ...
    net = Net(args.state_dim, hidden_sizes=args.hidden_sizes, device=args.device, activation=nn.Tanh)

    h_actor = ActorProb(net, args.state_dim, device=args.device, unbounded=True).to(args.device)
    h_critic = CCritic(
        net,
        device=args.device
    ).to(args.device)
    ...
    activation = nn.ReLU if args.env=='MovieLensEnv-v0' else nn.Tanh
    net = Net(args.state_dim + args.state_dim, hidden_sizes=args.hidden_sizes, device=args.device, activation=activation)
